...
import pauses.BasePause;
import pauses.Breakfast;
import pauses.Default;
import pauses.Pulse;
...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		else
		{
			...
			if (path != 'none') pauseMusic.loadEmbedded(Paths.music(path), true, true);
		}
		if (pauseEffect != null) add(pauseEffect);
		...
		switch (Paths.formatToSongPath(daSelected))
		{
			case 'skip-time':
			{
				...
				if (controls.UI_LEFT || controls.UI_RIGHT)
				{
					...
					if (curTime >= FlxG.sound.music.length) { curTime %= length; }
					else if (curTime < 0) curTime = Math.min((curTime % length) + length, length - 1000);
	...
	{
		pauseMusic.destroy();
		if (pauseEffect != null) { remove(pauseEffect); pauseEffect.destroy(); }
	...
	function regenMenu():Void
	{
		...
		{
			var item = new Alphabet(0, 70 * i + 30, menuItems[i], true, false, null, null);

...
class FlxInputText extends FlxText
{
	...
	private function onKeyDown(e:KeyboardEvent):Void
	{
		...
		if (hasFocus)
		{
			...
			if (key == 67 && #if macos e.commandKey #else e.ctrlKey #end)
			{
				Clipboard.text = text;
			...
			if (key == 86 && #if macos e.commandKey #else e.ctrlKey #end)
			{
			...
			if (key == 88 && #if macos e.commandKey #else e.ctrlKey #end)
			{
				Clipboard.text = text;

...
class ChartingState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		['Popup', "Causes a popup to spawn 50% of\nthe time this event is triggered."],
		[
			'Vignette',
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var player2DropDown = new FlxUIDropDownMenuCustom(player1DropDown.x, player1DropDown.y + 40,
		...
		var reloadNotesButton:FlxButton = new FlxButton(noteSplashesInputText.x + 5, noteSplashesInputText.y + 20, 'Change Notes', function()
		{
			...
			if (text.length > 0) { _song.arrowSkin = text; }
			else if (_song.arrowSkin != null) { _song.arrowSkin = null; }
	...
	function addSectionUI():Void
	{
		...
		if (check_changeBPM.checked) { stepperSectionBPM.value = _song.notes[curSec].bpm; }
		else { stepperSectionBPM.value = Conductor.bpm; }
		...
		var clearSectionButton:FlxButton = new FlxButton(pasteButton.x + 100, pasteButton.y, "Clear", function()
		{
			...
			if (check_eventsSec.checked)
			{
				...
				while (i > -1)
				{
					...
					if(event != null && endThing > event[0] && event[0] >= startThing) _song.events.remove(event);
					--i;
				}
			}
			updateGrid();
	...
	function addChartingUI()
	{
		...
		check_mute_inst.callback = function() { FlxG.sound.music.volume = CoolUtil.boolToInt(!check_mute_inst.checked); };
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			switch (wname)
			{
				...
				case 'voices_volume': vocals.volume = nums.value;
		...
		else if (id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText))
		{
			if (sender == noteSplashesInputText)
			{
				...
				if (text.length > 0) { _song.splashSkin = text; }
				else if (_song.splashSkin != null) { _song.splashSkin = null; }
			}
		...
		if (!blockInput)
		{
			...
			if (!vortex)
			{
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN)
				{
					...
					var fuck:Float = CoolUtil.quantize(beat, snap)
						+ (increase * (CoolUtil.boolToInt(FlxG.keys.justPressed.DOWN) - CoolUtil.boolToInt(FlxG.keys.justPressed.UP)));
					FlxG.sound.music.time = Conductor.beatToSeconds(fuck);
				}
			}
			...
			if (!blockInput)
			{
				curQuant = CoolUtil.repeat(curQuant, CoolUtil.boolToInt(FlxG.keys.justPressed.RIGHT) - CoolUtil.boolToInt(FlxG.keys.justPressed.LEFT),
					quantizations.length);
			...
			if (note.strumTime <= Conductor.songPosition)
			{
				...
				if (note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1)
				{
					...
					if (!playedSound[data])
					{
						...
						{
							var hitsound:FlxSound = Hitsound.play();
							if (hitsound != null)
	...
	function updateZoom()
	{
		...
		var zoomThing:String = (daZoom < 1) ? '${Math.round(1 / daZoom)} / 1' : '1 / $daZoom';
	...
	{
		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]), true, 0xFF090909, 0xFF2A2A2A);
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]), 0xFF090909, 0xFF2A2A2A);
	...
	function loadJson(song:String):Void
	{
		...
		if (diffSelected != null && CoolUtil.difficulties.length > 0 && diffSelected != CoolUtil.defaultDifficulty)
		{
		...
		else
		{
			...
			PlayState.SONG = Song.loadFromJson(formattedSong, path);
		}
		MusicBeatState.resetState();
	}
	...
	{
		if (Obj1 == null || Obj1[0] == null) return Obj2[0];
		if (Obj2 == null || Obj2[0] == null) return Obj1[0];

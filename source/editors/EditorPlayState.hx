...
class EditorPlayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		bg.scrollFactor.set();
		bg.color = FlxColor.fromHSB(FlxG.random.int(0, 359), FlxG.random.float(0, 0.8), FlxG.random.float(.3, 1));
		add(bg);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		var daRating:String = "funny";

		if (noteDiff > Conductor.safeZoneOffset * .75)
		{
			daRating = "horsedog";
			//score = 50;
		}
		else if (noteDiff > Conductor.safeZoneOffset * .5)
		{
			daRating = "bad";
			//score = 100;
		}
		else if (noteDiff > Conductor.safeZoneOffset * .25)
		{
			daRating = "goog";
			//score = 200;
		}

		if (daRating == "funny" && !note.noteSplashDisabled)
		{
			spawnNoteSplashOnNote(note);
		}
		//songScore += score;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player < 1 && ClientPrefs.middleScroll) targetAlpha = .35;

...
class Alphabet extends FlxSpriteGroup
{
	...
	public var typingSpeed:Float = .05;
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, typed:Bool = false, ?typingSpeed:Float = .05, ?textSize:Float = 1)
	{
		super(x, y);
	...
	public function startTypedText(speed:Float):Void
	{
		...
		if (speed <= 0) {
			...
			typeTimer = new FlxTimer().start(.1, function(tmr:FlxTimer) {
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if ((loopNum <= splitWords.length - 2 && splitWords[loopNum] == "\\" && splitWords[loopNum+1] == "n") ||
			((autoBreak = true) && xPos >= FlxG.width * .65 && splitWords[loopNum] == ' ' ))
		{
			if (autoBreak) {
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			...
			y = FlxMath.lerp(y, (scaledY * yMult) + (FlxG.height * .48) + yAdd, lerpVal);

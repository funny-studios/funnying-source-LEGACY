	...
	public function dance()
	{
		if (!debugMode && !specialAnim)
		{
			switch (danceIdle)
			{
				...
				{
					danced = !danced;
					playAnim(danced ? 'danceRight$idleSuffix' : 'danceLeft$idleSuffix', true);
				...
				default:
				{
					...
					if (animation.getByName(idleFormat) != null) playAnim(idleFormat, true);
				}
			}
		}
	}
			...
			default:
				...
				if (animationsArray != null && animationsArray.length > 0) {
					for (anim in animationsArray) {
						...
						if (animIndices != null && animIndices.length > 0) { animation.addByIndices(animAnim, animName, animIndices, "", animFps, animLoop); } else { animation.addByPrefix(animAnim, animName, animFps, animLoop); }
